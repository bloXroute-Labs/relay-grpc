...
func FuluRequestToProtoRequest(block *apiFulu.SubmitBlockRequest) *SubmitBlockRequest {
	...
	executionRequests := ConvertFuluExecutionRequestToProto(block.ExecutionRequests)

	return &SubmitBlockRequest{
		Version: uint64(consensusspec.DataVersionFulu),
		BidTrace: &BidTrace{
		...
		BlobsBundle:       convertFuluBlobBundleToProto(block.BlobsBundle),
...
func convertFuluBlobBundleToProto(blobBundle *apiFulu.BlobsBundle) *BlobsBundle {
...
// FuluRequestToProtoRequestWithShortIDs converts a fulu request to a SubmitBlockRequest.
func FuluRequestToProtoRequestWithShortIDs(block *apiFulu.SubmitBlockRequest, compressTxs []*CompressTx) *SubmitBlockRequest {
	withdrawals := make([]*Withdrawal, len(block.ExecutionPayload.Withdrawals))
	for i, withdrawal := range block.ExecutionPayload.Withdrawals {
		withdrawals[i] = &Withdrawal{
			ValidatorIndex: uint64(withdrawal.ValidatorIndex),
...
func ProtoRequestToFuluRequest(block *SubmitBlockRequest) (*apiFulu.SubmitBlockRequest, error) {
	...
	return &apiFulu.SubmitBlockRequest{
		Message: &v1.BidTrace{
		...
		ExecutionRequests: convertFuluProtoToExecutionRequest(block.ExecutionRequests),
		Signature:         b96(block.Signature),
	}, nil
}

func ProtoRequestToFuluBidtracePayload(block *SubmitBlockRequest) (*BidtracePayload, error) {
	value, err := uint256.FromHex(block.BidTrace.Value)
	if err != nil {
		return nil, fmt.Errorf("failed to convert fulu block value %s to uint256: %s", block.BidTrace.Value, err.Error())
	}
...
func ConvertFuluExecutionRequestToProto(executionRequests *electra.ExecutionRequests) *ExecutionRequests {
...
func convertFuluProtoToExecutionRequest(protoExecutionRequests *ExecutionRequests) *electra.ExecutionRequests {
...
func ProtoRequestToFuluHeaderSubmission(header *StreamHeaderResponse) (*SignedHeaderSubmissionFulu, error) {
	...
	executionRequests := convertFuluProtoToExecutionRequest(header.ExecutionRequests)

	return &SignedHeaderSubmissionFulu{
...
func FuluBlockRequestToHeaderSubmissionProtoRequest(block *apiFulu.SubmitBlockRequest, transactionsRoot []byte, withdrawalsRoot []byte) (*BidTrace, *ExecutionPayloadHeader, [][]byte, *ExecutionRequests, []byte) {
...
func ProtoRequestToFuluSignedBuilderBid(signedHeader *SignedBuilderBid) (*apiElectra.SignedBuilderBid, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("failed to convert fulu block value %s to uint256: %s", header.Value, err.Error())
	}
...
func FuluSignedBuilderBidToProtoRequest(signedBuilderBid *apiElectra.SignedBuilderBid) *SignedBuilderBid {
	...
	executionRequests := ConvertFuluExecutionRequestToProto(signedBuilderBid.Message.ExecutionRequests)
...
func FuluProtoRequestToBidTrace(bidTrace *BidTrace, value *uint256.Int) (*v1.BidTrace, error) {
	if bidTrace == nil {
...
func FuluBidTraceToProto(bidTrace *v1.BidTrace, blobGasUsed uint64, excessBlobGas uint64) *BidTrace {
	return &BidTrace{
		Slot:                 bidTrace.Slot,
